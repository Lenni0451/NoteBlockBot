...
public class RateLimiter {
    ...
    private static final LoadingCache<Long, Bucket> USER_BUCKETS = Caffeine.newBuilder().expireAfterAccess(Duration.ofMinutes(5)).build(key -> Bucket.builder()
            ...
            .build());
    private static final LoadingCache<Long, Bucket> GUILD_BUCKETS = Caffeine.newBuilder().expireAfterAccess(Duration.ofMinutes(5)).build(key -> Bucket.builder()
    ...
    public static boolean tryUser(final long userId) {
        return USER_BUCKETS.get(userId).tryConsume(1);
    }

    public static boolean tryGuild(final long guildId) {
        return GUILD_BUCKETS.get(guildId).tryConsume(1);
    }
